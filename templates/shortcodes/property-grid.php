 ...
 * 
 * This template displays the property grid.
 * 
 * HOW TO OVERRIDE THIS TEMPLATE:
 * 1. Copy this file from: wp-content/plugins/havenlytics/templates/shortcodes/property-grid.php
 * 2. Paste it to your theme at: wp-content/themes/your-theme/havenlytics/shortcodes/property-grid.php
 * 3. Modify the copied file to customize the property grid display
 * 
 * @package     Havenlytics
...
// Force total_pages to at least 1 if there are posts
if ($hvnly_total_properties > 0 && $hvnly_total_pages === 0) {
    $hvnly_total_pages = ceil($hvnly_total_properties / $hvnly_per_page);
}
...
$hvnly_container_classes = array_filter([
    ...
    'hvnly-grid-view',
    'hvnly-columns-' . absint($hvnly_atts['columns']),
...
?>
<div class="<?php echo esc_attr(implode(' ', $hvnly_container_classes)); ?>"
    <?php echo $hvnly_container_id; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
    data-shortcode="<?php echo esc_attr($hvnly_shortcode); ?>">

    <?php
    /**
     * Top Results Bar
     * 
     * Shows results count and pagination above the grid if enabled.
     */
    if ($hvnly_show_top_bar) :
        ...
        if ($hvnly_has_results_to_show || $hvnly_has_pagination_to_show) :
            hvnly_get_template('global/results-bar-shortcode', [
                ...
                'context'          => 'shortcode',
                'position'         => 'top',
                'classes'          => 'hvnly-property-grid__results-bar-top',
            ]);
        endif;
    endif;
    ?>
    ...
    <div class="hvnly-property-archive-container">
        ...
        <div class="hvnly-property-grid"
            style="grid-template-columns: repeat(<?php echo absint($hvnly_atts['columns']); ?>, minmax(0, 1fr)); gap: 20px;">

            <?php if ($hvnly_properties->have_posts()) : ?>
            <?php while ($hvnly_properties->have_posts()) : $hvnly_properties->the_post(); ?>
            <?php 
                    /**
                     * Render property card using the existing template system.
                     * Falls back to content-property template if render function doesn't exist.
                     */
     ...
     * Bottom Results Bar
     * 
     * Shows results count and pagination below the grid if enabled.
     */
    if ($hvnly_show_bottom_bar) :
        ...
        if ($hvnly_has_results_to_show || $hvnly_has_pagination_to_show) :
            hvnly_get_template('global/results-bar-shortcode', [
                ...
                'context'          => 'shortcode',
                'position'         => 'bottom',
                'classes'          => 'hvnly-property-grid__results-bar',
            ]);
        endif;
    endif;
    ?>
