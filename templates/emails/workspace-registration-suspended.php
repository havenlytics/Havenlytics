 ...
 *
 * Sprint 24C: retrofitted onto the shared email layout. The template slug and
 * its context variables ($user_name, $intro, $workspace_url, $site_name) are
 * unchanged, so existing theme overrides and the `hvnly_email_templates` map
...
/*
 * Variables are listed explicitly. get_defined_vars() cannot be used here:
 * hvnly_get_template() extract()s $args into template scope, so get_defined_vars()
 * would also capture $args, $located and $template_name — re-nesting the whole
 * context on every partial hop until memory is exhausted.
 *
 * No CTA: a suspended agent cannot sign in, so a sign-in button would be a dead end.
 */
$hvnly_vars = array(
	'title'             => isset( $title ) ? (string) $title : __( 'Your account has been paused', 'havenlytics' ),
