...
/**
 * Whether the global share popup markup/script should load (live frontend only).
 *
 * @return bool
 */
function hvnly_should_render_share_popup()
{
    return ! hvnly_is_elementor_canvas_context();
}

/**
 * Whether property listing pages should load share modal assets.
 *
 * @return bool
 */
function hvnly_should_load_share_modal_assets()
{
    ...
    if ( ! hvnly_should_render_share_popup() ) {
        return false;
    }
    ...
    $should_load = is_post_type_archive( 'hvnly_property' )
        || is_singular( array( 'hvnly_agent', 'hvnly_agency' ) )
        || is_post_type_archive( array( 'hvnly_agent', 'hvnly_agency' ) );

    if ( is_tax( 'hvnly_agent_agency' ) ) {
        $is_agency_index = function_exists( 'hvnly_is_agency_archive_index' ) && hvnly_is_agency_archive_index();
        if ( ! $is_agency_index ) {
            $should_load = true;
        }
    }

    $property_taxonomies = get_object_taxonomies( 'hvnly_property', 'names' );
    if ( ! empty( $property_taxonomies ) && is_tax( $property_taxonomies ) ) {
        $should_load = true;
    }

    if ( function_exists( 'hvnly_page_has_property_shortcode' ) && hvnly_page_has_property_shortcode() ) {
        $should_load = true;
    }

    if ( function_exists( 'hvnly_page_has_property_shortcodes' ) && hvnly_page_has_property_shortcodes() ) {
        $should_load = true;
    }

    return (bool) apply_filters( 'hvnly_should_load_share_modal_assets', $should_load );
}

/**
 * Register footer output for the global share popup (once per request).
 *
 * @return void
 */
function hvnly_boot_share_popup()
{
    if ( ! function_exists( 'hvnly_is_share_button_enabled' ) || ! hvnly_is_share_button_enabled() ) {
        return;
    }
    ...
    if ( ! has_action( 'wp_footer', 'hvnly_render_global_share_popup' ) ) {
        add_action( 'wp_footer', 'hvnly_render_global_share_popup', 20 );
    }
}

/**
 * Output global share popup markup in the footer.
 *
 * @return void
 */
function hvnly_render_global_share_popup()
{
    static $rendered = false;

    if ( $rendered || ! hvnly_should_render_share_popup() ) {
        return;
    }

    $rendered = true;
    hvnly_get_template_part( 'global/share-popup' );
}
