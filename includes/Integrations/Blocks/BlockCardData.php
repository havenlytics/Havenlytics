...
final class BlockCardData {
    ...
    public static function get( int $id ): array {
        $base = function_exists('hvnly_get_property_data') ? (array) hvnly_get_property_data($id) : array();
        ...
        if ('' === $thumb) {
            $gallery = isset($base['gallery_image_ids']) ? (array) $base['gallery_image_ids'] : array();
            if ( ! empty($gallery)) {
    ...
    private static function term_names( int $id, string $tax ): array {
        ...
        return ( ! is_wp_error($terms) && is_array($terms) ) ? array_values($terms) : array();
    }
}
