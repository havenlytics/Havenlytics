...
class DnDSections {
    ...
    private $generated_base_ids  = array();
    private $generated_group_ids = array();
    ...
    public function routes() {
        ...
        register_rest_route($this->namespace, '/' . $this->route_base . '/reset-unified', array(
            'methods' => 'POST',
            'callback' => array( $this, 'reset_unified' ),
            'permission_callback' => array( $this, 'create_per' ),
        ));

        register_rest_route($this->namespace, '/master-base-ids', array(
            'methods' => 'GET',
            'callback' => array( $this, 'get_master_base_ids' ),
            'permission_callback' => array( $this, 'get_per' ),
        ));

        register_rest_route($this->namespace, '/generate-unique-ids', array(
            'methods' => 'GET',
            'callback' => array( $this, 'generate_unique_ids' ),
            'permission_callback' => array( $this, 'get_per' ),
        ));
    }
    ...
    private function reset_generated_ids() {
        $this->generated_base_ids  = array();
        $this->generated_group_ids = array();
    }
    ...
    public function save_all( $request ) {
        try {
            ...
            if ( ! isset($params['tabs'])) {
                return new \WP_Error('invalid_data', 'Tabs data is required', array( 'status' => 400 ));
            }

            if ( ! is_array($params['tabs'])) {
                return new \WP_Error('invalid_data', 'Tabs must be an array', array( 'status' => 400 ));
            }
    ...
    private function sanitize_fields_with_groups_and_normalize( $fields, $master_ids ) {
        ...
        foreach ($fields as $field) {
            ...
            if ( ! empty($field['condition'])) {
                $sanitized_field['condition'] = array(
    ...
    public static function ensure_basic_info_fields( $stored_fields ) {
        ...
        $stored   = is_array( $stored_fields ) ? $stored_fields : array();
        $merged   = array();
        ...
        foreach ( $defaults as $default_field ) {
            ...
            if ( $match ) {
                ...
                $merged[] = array_merge( $default_field, $match, array(
                    'id'      => $default_field['id'],
    ...
    private function get_address_neighborhood_fields_fallback() {
        return array(
            ...
            array(
				...
				'enabled' => true,
				'order' => 8,
				'options' => function_exists( 'hvnly_get_location_field_options' ) ? hvnly_get_location_field_options() : array(),
			),
            array(
				...
				'enabled' => true,
				'order' => 9,
				'options' => function_exists( 'hvnly_get_country_field_options' ) ? hvnly_get_country_field_options() : array(),
			),
        );
    }
