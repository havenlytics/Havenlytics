...
final class Assets {
     ...
     */
    private const REACT_BUILD_PAGES = array(
        'hvnly_property_page_hvnly_property_settings' => array(
            'folder' => 'settings',
            'asset'  => 'settings.asset.php',
            'script' => '0.js',
            'style'  => '0.css',
        ),
        'hvnly_property_page_hvnly_property_builder' => array(
            'folder' => 'builder',
            'asset'  => 'builder.asset.php',
            'script' => '0.js',
            'style'  => '0.css',
        ),
        'toplevel_page_hvnly_property_builder' => array(
            'folder' => 'builder',
            'asset'  => 'builder.asset.php',
            'script' => '0.js',
            'style'  => '0.css',
        ),
        'hvnly_property_page_hvnly_property_reports_analytics' => array(
            'folder' => 'reports',
            'asset'  => 'reports.asset.php',
            'script' => '0.js',
            'style'  => '0.css',
        ),
        'toplevel_page_hvnly_property_reports_analytics' => array(
            'folder' => 'reports',
            'asset'  => 'reports.asset.php',
            'script' => '0.js',
            'style'  => '0.css',
     ...
     */
    private const REPORTS_PAGE_ASSETS = array(
    ...
    public function __construct() {
        ...
        add_action('admin_notices', array( self::class, 'render_missing_build_asset_notice' ));
    }
    ...
    private function enqueue_setup_assets( $version ): void {
        ...
        wp_localize_script('hvnly-setup-wizard', 'hvnlysetupWizard', array(
    ...
    private function enqueue_page_assets( $page, $version ): void {
        ...
        $dependencies  = $this->normalize_script_dependencies($main_asset['dependencies'] ?? array());
        ...
        if (file_exists($css_file)) {
            wp_enqueue_style(
                ...
                array_merge(array( 'hvnly-admin-boot' ), $chunk_handles['styles']),
                $main_asset['version'] ?? $version
            );
        }
    ...
    private function enqueue_numeric_webpack_chunks(
        ...
        foreach (scandir($build_dir) ?: array() as $file) {
        ...
        foreach ($script_files as $index => $chunk_file) {
            ...
            wp_enqueue_script(
                ...
                $prev_script_handle === '' ? $dependencies : array( $prev_script_handle ),
                $version,
                true
            );
        ...
        foreach ($style_files as $index => $chunk_file) {
            ...
            wp_enqueue_style(
                $chunk_handle,
                HVNLYNAB_BUILD_URL . '/' . $folder . '/' . $chunk_file,
                $prev_style_handle === '' ? array() : array( $prev_style_handle ),
                $version
            );
