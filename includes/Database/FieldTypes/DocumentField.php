...
<div class="hvnly-document-field-container" data-field-id="<?php echo esc_attr($field['id']); ?>"
    ...
    <div class="hvnly-document-repeater-items">
		...
		if ( ! empty($documents)) {
			...
			$this->render_document_item($field_base, 0, array(
				'icon' => '',
				'label' => '',
				'url' => '',
				'url_type' => 'custom',
...
<div class="hvnly-document-repeater-item" data-item-index="<?php echo esc_attr($index); ?>">
    ...
    private function get_documents( $post_id, $field_name, $field_base, $field = array() ) {
        ...
        if ( ! empty($icon_value) || ! empty($label_value) || ! empty($url_value)) {
            ...
            if ( ! is_array($icon_value) && ! is_array($label_value) && ! is_array($url_value)) {
                ...
                $icons  = is_array($icon_value) ? $icon_value : array( $icon_value );
                $labels = is_array($label_value) ? $label_value : array( $label_value );
                $urls   = is_array($url_value) ? $url_value : array( $url_value );
    ...
    public function save( $post_id, $field_name, $value = null, $extra = null ) {
        ...
        $icons_raw     = filter_input(INPUT_POST, $field_name . '_icons', FILTER_UNSAFE_RAW, FILTER_REQUIRE_ARRAY) ?: array();
        $labels_raw    = filter_input(INPUT_POST, $field_name . '_labels', FILTER_UNSAFE_RAW, FILTER_REQUIRE_ARRAY) ?: array();
        $urls_raw      = filter_input(INPUT_POST, $field_name . '_urls', FILTER_UNSAFE_RAW, FILTER_REQUIRE_ARRAY) ?: array();
        $url_types_raw = filter_input(INPUT_POST, $field_name . '_url_types', FILTER_UNSAFE_RAW, FILTER_REQUIRE_ARRAY) ?: array();
    ...
    public function validate( $value, $field ) {
        ...
        $labels_raw = filter_input(INPUT_POST, $field['name'] . '_labels', FILTER_UNSAFE_RAW, FILTER_REQUIRE_ARRAY) ?: array();
        $urls_raw   = filter_input(INPUT_POST, $field['name'] . '_urls', FILTER_UNSAFE_RAW, FILTER_REQUIRE_ARRAY) ?: array();
