...
class SidebarSearchFilters {
    ...
    private function generate_sidebar_filter_data() {
        ...
        $data['current_amenities']       = $data['current_filters']['amenities'] ?? array();
    ...
    private function get_current_property_id_values( $current_filters ) {
        $current_values = $current_filters['property_ids'] ?? array();
    ...
    private function get_current_taxonomy_values( $taxonomy, $current_filters ) {
        $current_values = $current_filters[ $taxonomy ] ?? array();
    ...
    public function build_sidebar_query_args( $data, $page = 1, $per_page = 12 ) {
        ...
        if ( ! empty($tax_query)) {
            $args['tax_query'] = count($tax_query) > 1 ? array_merge($tax_query, array( 'relation' => 'AND' )) : $tax_query;
        }
        ...
        if ( ! empty($meta_query)) {
            $args['meta_query'] = count($meta_query) > 1 ? array_merge($meta_query, array( 'relation' => 'AND' )) : $meta_query;
        }
