...
abstract class AbstractShortcode {
     ...
     * @var array
     */
    private static $active_shortcodes = [];
    ...
    public function __construct() {
        ...
        if (!self::$body_class_filter_added) {
            add_filter('body_class', [__CLASS__, 'add_shortcode_body_classes'], PHP_INT_MAX);
            self::$body_class_filter_added = true;
        }
        
        // Also try to catch via template_redirect (even later)
        add_action('template_redirect', [$this, 'capture_shortcodes_in_content'], 0);
    }
    ...
    protected function before_render() {
        ...
        if (!isset($GLOBALS['hvnly_shortcode_stack'])) {
            $GLOBALS['hvnly_shortcode_stack'] = [];
        }
        
        // Push current state to stack
        array_push($GLOBALS['hvnly_shortcode_stack'], [
     ...
     * @return array
     */
    protected function get_template_args($atts, $additional = []) {
        return array_merge([
            'shortcode'   => $this->tag,
            'atts'        => $atts,
            'is_shortcode' => true,
